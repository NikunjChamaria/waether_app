...
class _HomePageState extends State<HomePage> {
  ...
  late Future<dynamic> yourLocation;
  @override
  void initState() {
    //clearlocal();
    yourLocation = getLocation();
    getSavedLocationlength();
    super.initState();
  }
  ...
  Future<void> getSavedLocationlength() async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    savedLocations = sharedPreferences.getStringList("savedLocation") ?? [];
    locations.addAll(sharedPreferences.getStringList("savedLocation") ?? []);
  }
                                  ...
                                  children: [
                                    ...
                                    Padding(
                                      padding: EdgeInsets.only(
                                          top: 130.h, left: 30.w, right: 30.w),
                                      child: Column(
                                        crossAxisAlignment:
          ...
          Padding(
            ...
            child: Container(
                ...
                child: TextFormField(
                  ...
                  onEditingComplete: () {
                    setState(() {
                      locations.add(search.text);
                      print(locations);
                      pageController.jumpToPage(locations.length);
                    });
                  },
                  style: poppins(Colors.black, 16.sp, FontWeight.normal),
                  decoration: InputDecoration(
                      border: InputBorder.none,
                      prefixIcon: const Icon(Icons.search),
                      hintText: "Search",
                      hintStyle:
                          poppins(Colors.grey, 16.sp, FontWeight.normal)),
                )),
          ),
          Padding(
        ...
        children: [
          FloatingActionButton(
              ...
              onPressed: () async {
                ...
                setState(() {
                  List<String> data =
                      sharedPreferences.getStringList("savedLocation") ?? [];
                  data.add(locations[currIndex]);
                  savedLocations.add(locations[currIndex]);
                  sharedPreferences.setStringList("savedLocation", data);
                  print(sharedPreferences.getStringList("savedLocation") ?? []);
                });
              },
              child: Icon(
                Icons.favorite,
                size: 24.sp,
                color: savedLocations.contains(locations[currIndex])
                    ? Colors.pink
                    : Colors.black,
              )),
        ],
